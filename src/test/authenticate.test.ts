...
describe('authenticate', async () => {
  it('should handle faulty requests', async () => {
    ...
    await authenticator
      ...
      .catch(async (e) =>
        expect((await e.json())?.message).toEqual(
          'Need a valid email and/or password'
        )
      )
  })
  ...
  it('should sign in and return the session', async () => {
    const fData = new FormData()
    ...
    await authenticator
      ...
      .then((res) => expect(res).toEqual(validSession))
  })
})
