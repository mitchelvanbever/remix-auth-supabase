...
export const validResponse = {
  refresh_token: 'valid', access_token: 'valid',
}
  ...
  rest.post('http://supabase-url.com/supabase-project/auth/v1/token', async(req, res, ctx) => {
    ...
    if (refresh_token) {
      if (refresh_token === 'userA')
        return res(ctx.status(200), ctx.json({ ...validResponse, ...concurrentUserA }))

      if (refresh_token === 'userB')
        return res(ctx.status(200), ctx.json({ ...validResponse, ...concurrentUserB }))
