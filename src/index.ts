...
export class SupabaseStrategy extends Strategy<Session, VerifyParams> {
  private static pendingRefreshes: Map<
    string,
    Promise<{
      data: Session | null;
      error: ApiError | null;
  ...
  constructor(options: SupabaseStrategyOptions, verify: StrategyVerifyCallback<Session, VerifyParams>) {
  ...
  protected async handleResult(req: Request, options: AuthenticateOptions, result: any, hasErrored = false) {
    if (options.failureRedirect && hasErrored) return this.failure(result, req, this.sessionStorage, options);

    if (hasErrored) return null;

    if (options.successRedirect && !hasErrored) return this.success(result, req, this.sessionStorage, options);
  ...
  async checkSession(req: Request, checkOptions: CheckOptions = {}): Promise<Session | null> {
    ...
    if (!user || user?.error) {
      ...
      if (pendingRefresh) {
        ...
        return this.success(res.data, req, this.sessionStorage, {
          ...options,
